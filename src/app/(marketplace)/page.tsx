...
export default async function LandingPage() {
    ...
    const landingBlocks: any[] = brand.isSingleVendor 
        ? (brand as any).landingBlocks || []
        : (siteSettings?.data as any)?.landingBlocks || [];
    ...
    return (
        ...
        <div className="flex flex-col">
            {landingBlocks.length > 0 ? (
                <>
                {landingBlocks.map((block: any, i: number) => {
                    if (block.type === 'heroSlider') {
                        const sliderImgs = block.images && block.images.length > 0 ? block.images : heroImages;
                        return (
                            <div key={i}>
                                <section className="relative h-[56.25vw] min-h-[70vh] max-h-[95vh] flex items-center overflow-hidden" suppressHydrationWarning>
                <HeroSlider
                    images={sliderImgs}
                ...
                <div className={`max-w-7xl mx-auto px-6 relative z-10 w-full h-full flex flex-col ${
                    block.verticalAlign === 'top' ? 'justify-start pt-16 md:pt-24 pb-24 md:pb-32' :
                    block.verticalAlign === 'bottom' ? 'justify-end pb-32 md:pb-40 pt-16 md:pt-24' :
                    'justify-center pt-16 pb-28 md:pt-20 md:pb-36'
                }`}>
                    <div className={`max-w-2xl space-y-4 sm:space-y-6 md:space-y-8 flex flex-col ${
                        block.align === 'center' ? 'items-center text-center mx-auto' :
                        block.align === 'right' ? 'items-end text-right ml-auto' :
                        'items-start text-left'
                    }`}>
                        {/* Banner temporada */}
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-white text-[10px] sm:text-xs font-black uppercase tracking-widest" style={{ background: "linear-gradient(to right, var(--brand-600), var(--brand-700))", boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
                            {block.badgeText ? (
                                ...
                                <>
                                    ...
                                    {stats.newThisWeek > 0 && (
                                        <span className="ml-1 px-1.5 py-0.5 sm:px-2 bg-white/20 rounded-full text-[8px] sm:text-[9px]">
                        ...
                        <div className="space-y-2 sm:space-y-3 md:space-y-4">
                            <h2 className="font-black uppercase tracking-[0.2em] text-[10px] sm:text-xs md:text-sm drop-shadow-sm" style={{ color: "var(--brand-500)" }}>
                                {block.subtitle !== undefined && block.subtitle !== ''
                                    ? block.subtitle 
                                    : (brand.isSingleVendor ? brand.tagline : `${brand.name} Fashion Marketplace`)}
                            </h2>
                            <h1 className={`${block.textSize === 'compact' ? 'text-3xl sm:text-4xl lg:text-5xl xl:text-6xl' : 'text-4xl sm:text-6xl lg:text-7xl xl:text-8xl'} font-black tracking-tighter leading-none md:leading-[0.85] text-foreground drop-shadow-sm pb-2`}>
                        ...
                        <p className="hidden sm:block text-sm md:text-lg lg:text-xl text-foreground/80 dark:text-white/80 font-medium max-w-lg leading-relaxed drop-shadow-sm">
                        ...
                        <div className="flex flex-wrap gap-4 pt-2 sm:pt-4">
                            <Link href={block.ctaLink || "/catalog"} className="px-6 py-3 sm:px-8 sm:py-4 text-white rounded-full text-xs sm:text-sm font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl" style={{ backgroundColor: "var(--brand-600)" }}>
                                {block.ctaText || "Explorar Catálogo"}
                            </Link>
                ...
                <div className="absolute bottom-0 left-0 right-0 z-10 bg-background/80 backdrop-blur-md border-t border-border">
                    <div className="max-w-7xl mx-auto px-6 py-3 sm:py-5">
                        <div className="flex flex-wrap gap-4 sm:gap-6 md:gap-16 items-center text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">
                            {!brand.isSingleVendor && stats.totalSellers >= 50 && (
                                <div className="flex items-center gap-2 sm:gap-3">
                                    <span className="text-xl sm:text-2xl font-black text-foreground">{stats.totalSellers}</span>
                                    <span>Vendedores<br />verificados</span>
                                </div>
                            )}
                            <div className={`flex items-center gap-2 sm:gap-3 ${!brand.isSingleVendor ? 'md:border-l md:border-border md:pl-16' : ''}`}>
                                <span className="text-xl sm:text-2xl font-black text-foreground">{stats.totalProducts.toLocaleString()}</span>
                                <span>Productos<br />en catálogo</span>
                            </div>
                            <div className="flex items-center gap-2 sm:gap-3 md:border-l md:border-border md:pl-16">
                                <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-500 animate-pulse inline-block" />
                                <span className="text-xl sm:text-2xl font-black text-emerald-500">{stats.newThisWeek}</span>
                                <span>Nuevos<br />esta semana</span>
                            </div>
                            {brand.isSingleVendor && brand.whatsapp && (
                                <div className="flex items-center gap-2 sm:gap-3 md:border-l md:border-border md:pl-16">
                                    <a href={`https://wa.me/${brand.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform" style={{backgroundColor: brand.isSingleVendor ? '#25D366' : 'var(--brand-600)'}}>
                                        📱 WhatsApp
                                    </a>
                                </div>
                            )}
                            {!brand.isSingleVendor && stats.totalOrders >= 100 && (
                                <div className="flex items-center gap-2 sm:gap-3 md:border-l md:border-border md:pl-16">
                                    <span className="text-xl sm:text-2xl font-black text-foreground">{stats.totalOrders.toLocaleString()}</span>
                    ...
                    if (block.type === 'featuredCategories') {
                        return (
                            <div key={i}>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                    <div className="space-y-2">
                        ...
                        <h2 className="text-4xl font-black tracking-tight uppercase">{block.title || "EXPLORA POR CATEGORÍA"}</h2>
                    </div>
                ...
                <div className="max-w-7xl mx-auto px-6">
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                        <div className="space-y-2">
                            ...
                            <h2 className="text-4xl font-black tracking-tight uppercase">{block.title || "NUEVOS MODELOS"}</h2>
                        </div>
                                ...
                                {block.imageUrl && (
                                    block.linkUrl ? (
                                        <Link href={block.linkUrl}>
                                            <img src={block.imageUrl} alt={block.altText || 'Banner'} className="w-full object-cover" />
                                        </Link>
                                    ) : (
                                        <img src={block.imageUrl} alt={block.altText || 'Banner'} className="w-full object-cover" />
                                    )
                ...
                {/* ── DESTACADOS — Vendedores y Productos (incluso con bloques custom) ── */}
                ...
                {/* Siempre mostramos RecentlyViewed al final si hay bloques custom */}
                <RecentlyViewed />
                </>
            ) : (
                <>
{/* ── HERO ── */}
            <section className="relative h-[56.25vw] min-h-[70vh] max-h-[95vh] flex items-center overflow-hidden" suppressHydrationWarning>
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10 w-full h-full flex flex-col justify-center pt-16 pb-28 md:pt-20 md:pb-36">
                    <div className="max-w-2xl space-y-4 sm:space-y-6 md:space-y-8 flex flex-col items-start text-left">
                        {/* Banner temporada */}
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-white text-[10px] sm:text-xs font-black uppercase tracking-widest" style={{ background: "linear-gradient(to right, var(--brand-600), var(--brand-700))", boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
                            <span>{season.emoji}</span>
                            <span>Tendencias {season.name}</span>
                        ...
                        <div className="space-y-2 sm:space-y-3 md:space-y-4">
                            <h2 className="font-black uppercase tracking-[0.2em] text-[10px] sm:text-xs md:text-sm drop-shadow-sm" style={{ color: "var(--brand-500)" }}>{brand.isSingleVendor ? brand.tagline : `${brand.name} Fashion Marketplace`}</h2>
                            <h1 className="text-4xl sm:text-6xl lg:text-7xl xl:text-8xl font-black tracking-tighter leading-none md:leading-[0.85] text-foreground drop-shadow-sm pb-2">
                                {brand.isSingleVendor ? (
                                    <>{brand.name.split(' ')[0].toUpperCase()} <br /><span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(to right, var(--brand-600), var(--brand-700))" }}>{brand.name.split(' ').slice(1).join(' ').toUpperCase() || 'FASHION'}.</span></>
                                ) : (
                                    <>LA MODA <br />QUE MUEVE A <br /><span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(to right, var(--brand-600), var(--brand-700))" }}>MÉXICO.</span></>
                        ...
                        <div className="flex flex-wrap gap-4 pt-2 sm:pt-4">
                            <Link href="/catalog" className="px-6 py-3 sm:px-8 sm:py-4 text-white rounded-full text-xs sm:text-sm font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl" style={{ backgroundColor: "var(--brand-600)" }}>
                                Explorar Catálogo
                            </Link>
